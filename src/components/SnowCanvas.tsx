import { useEffect, useRef } from 'react'
import { useCosmicTheme } from '@/hooks/useCosmicTheme'
import './SnowCanvas.css'

const nightColors: number[][] = [
  [255, 255, 255],
  [200, 210, 255],
  [180, 190, 247],
  [220, 200, 255]
]

const duskColors: number[][] = [
  [200, 160, 80],
  [180, 140, 60],
  [210, 170, 90],
  [170, 130, 50]
]

class Star {
  x = 0
  y = 0
  radius = 0
  baseOpacity = 0
  opacity = 0
  drift = 0
  sway = 0
  swaySpeed = 0
  twinkleSpeed = 0
  twinklePhase = 0
  speed = 0
  color: number[] = [255, 255, 255]

  constructor(w: number, h: number, isDusk: boolean) {
    this.reset(w, h, true, isDusk)
  }

  reset(w: number, h: number, initial = false, isDusk = false) {
    this.x = Math.random() * w
    this.radius = isDusk ? Math.random() * 2.2 + 0.8 : Math.random() * 1.8 + 0.5
    this.baseOpacity = isDusk ? Math.random() * 0.25 + 0.08 : Math.random() * 0.5 + 0.2
    this.opacity = this.baseOpacity
    this.drift = Math.random() * 0.2 - 0.1
    this.sway = Math.random() * Math.PI * 2
    this.swaySpeed = Math.random() * 0.008 + 0.003
    this.twinkleSpeed = Math.random() * 0.03 + 0.01
    this.twinklePhase = Math.random() * Math.PI * 2

    if (isDusk) {
      this.y = initial ? Math.random() * h : h + 10
      this.speed = -(Math.random() * 0.3 + 0.08)
      this.color = duskColors[Math.floor(Math.random() * duskColors.length)]
    } else {
      this.y = initial ? Math.random() * h : -10
      this.speed = Math.random() * 0.3 + 0.05
      this.color = nightColors[Math.floor(Math.random() * nightColors.length)]
    }
  }

  update(w: number, h: number, isDusk: boolean, mouseX: number, mouseY: number) {
    this.y += this.speed
    this.sway += this.swaySpeed
    this.x += this.drift + Math.sin(this.sway) * 0.2
    this.twinklePhase += this.twinkleSpeed
    this.opacity = this.baseOpacity + Math.sin(this.twinklePhase) * 0.15

    if (mouseX >= 0 && mouseY >= 0) {
      const dx = this.x - mouseX
      const dy = this.y - mouseY
      const dist = Math.sqrt(dx * dx + dy * dy)
      if (dist < 150) {
        const boost = (1 - dist / 150) * 0.4
        this.opacity = Math.min(1, this.opacity + boost)
      }
    }

    if (isDusk) {
      if (this.y < -10 || this.x < -10 || this.x > w + 10) {
        this.reset(w, h, false, true)
      }
    } else {
      if (this.y > h + 10 || this.x < -10 || this.x > w + 10) {
        this.reset(w, h, false, false)
      }
    }
  }

  draw(ctx: CanvasRenderingContext2D) {
    const [r, g, b] = this.color
    if (this.radius > 1.2) {
      ctx.beginPath()
      ctx.arc(this.x, this.y, this.radius * 3, 0, Math.PI * 2)
      ctx.fillStyle = `rgba(${r}, ${g}, ${b}, ${this.opacity * 0.08})`
      ctx.fill()
    }
    ctx.beginPath()
    ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2)
    ctx.fillStyle = `rgba(${r}, ${g}, ${b}, ${this.opacity})`
    ctx.fill()
  }
}

class ShootingStar {
  x = 0
  y = 0
  len = 0
  speed = 0
  angle = 0
  opacity = 1
  decay = 0
  active = true

  constructor(w: number, h: number) {
    this.x = Math.random() * w * 0.7
    this.y = Math.random() * h * 0.4
    this.len = Math.random() * 80 + 40
    this.speed = Math.random() * 8 + 6
    this.angle = (Math.random() * 0.4 + 0.2) * Math.PI
    this.decay = Math.random() * 0.015 + 0.01
  }

  update() {
    this.x += Math.cos(this.angle) * this.speed
    this.y += Math.sin(this.angle) * this.speed
    this.opacity -= this.decay
    if (this.opacity <= 0) this.active = false
  }

  draw(ctx: CanvasRenderingContext2D) {
    const tailX = this.x - Math.cos(this.angle) * this.len
    const tailY = this.y - Math.sin(this.angle) * this.len
    const grad = ctx.createLinearGradient(tailX, tailY, this.x, this.y)
    grad.addColorStop(0, `rgba(200, 210, 255, 0)`)
    grad.addColorStop(1, `rgba(200, 210, 255, ${this.opacity})`)
    ctx.beginPath()
    ctx.moveTo(tailX, tailY)
    ctx.lineTo(this.x, this.y)
    ctx.strokeStyle = grad
    ctx.lineWidth = 1.5
    ctx.stroke()
  }
}

class FloatingOrb {
  x = 0
  y = 0
  radius = 0
  speed = 0
  opacity = 0
  drift = 0
  sway = 0
  active = true

  constructor(w: number, h: number) {
    this.x = Math.random() * w
    this.y = h * 0.5 + Math.random() * h * 0.5
    this.radius = Math.random() * 16 + 8
    this.speed = Math.random() * 0.3 + 0.15
    this.opacity = Math.random() * 0.06 + 0.02
    this.drift = Math.random() * 0.3 - 0.15
    this.sway = Math.random() * Math.PI * 2
  }

  update() {
    this.y -= this.speed
    this.sway += 0.006
    this.x += this.drift + Math.sin(this.sway) * 0.3
    this.opacity -= 0.0002
    if (this.opacity <= 0 || this.y < -30) this.active = false
  }

  draw(ctx: CanvasRenderingContext2D) {
    const grad = ctx.createRadialGradient(this.x, this.y, 0, this.x, this.y, this.radius)
    grad.addColorStop(0, `rgba(200, 150, 60, ${this.opacity})`)
    grad.addColorStop(0.5, `rgba(190, 140, 50, ${this.opacity * 0.3})`)
    grad.addColorStop(1, `rgba(180, 130, 40, 0)`)
    ctx.beginPath()
    ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2)
    ctx.fillStyle = grad
    ctx.fill()
  }
}

type Effect = ShootingStar | FloatingOrb

export default function SnowCanvas() {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const { theme } = useCosmicTheme()
  const themeRef = useRef(theme)

  useEffect(() => {
    themeRef.current = theme
  }, [theme])

  useEffect(() => {
    const el = canvasRef.current
    if (!el) return
    const ctx = el.getContext('2d')
    if (!ctx) return

    let animationId = 0
    let particles: Star[] = []
    let effects: Effect[] = []
    let mouseX = -1
    let mouseY = -1
    let currentTheme = themeRef.current
    let canvasOpacity = 1
    let fadeTarget = 1
    let effectTimer = 0

    const resize = () => {
      el.width = window.innerWidth
      el.height = window.innerHeight
    }

    const rebuildParticles = (w: number, h: number, isDusk: boolean) => {
      const count = w < 768 ? 40 : 80
      particles = Array.from({ length: count }, () => new Star(w, h, isDusk))
      effects = []
    }

    const onMouseMove = (e: MouseEvent) => {
      mouseX = e.clientX
      mouseY = e.clientY
    }
    const onMouseLeave = () => {
      mouseX = -1
      mouseY = -1
    }

    resize()
    window.addEventListener('resize', resize)
    window.addEventListener('mousemove', onMouseMove)
    window.addEventListener('mouseleave', onMouseLeave)

    rebuildParticles(el.width, el.height, currentTheme === 'dusk')

    const animate = () => {
      // trigger fade when theme changes
      const latest = themeRef.current
      if (latest !== currentTheme && fadeTarget === 1) {
        fadeTarget = 0
      }

      if (canvasOpacity !== fadeTarget) {
        const step = 0.04
        if (fadeTarget < canvasOpacity) {
          canvasOpacity = Math.max(fadeTarget, canvasOpacity - step)
        } else {
          canvasOpacity = Math.min(fadeTarget, canvasOpacity + step)
        }

        if (canvasOpacity <= 0.01 && fadeTarget === 0) {
          currentTheme = themeRef.current
          rebuildParticles(el.width, el.height, currentTheme === 'dusk')
          fadeTarget = 1
        }
      }

      el.style.opacity = String(canvasOpacity)
      ctx.clearRect(0, 0, el.width, el.height)

      const isDusk = currentTheme === 'dusk'

      for (const p of particles) {
        p.update(el.width, el.height, isDusk, mouseX, mouseY)
        p.draw(ctx)
      }

      effectTimer++
      if (isDusk) {
        if (effectTimer > 120 && Math.random() < 0.015) {
          effects.push(new FloatingOrb(el.width, el.height))
          effectTimer = 0
        }
      } else {
        if (effectTimer > 300 && Math.random() < 0.008) {
          effects.push(new ShootingStar(el.width, el.height))
          effectTimer = 0
        }
      }

      for (let i = effects.length - 1; i >= 0; i--) {
        effects[i].update()
        effects[i].draw(ctx)
        if (!effects[i].active) effects.splice(i, 1)
      }

      animationId = requestAnimationFrame(animate)
    }

    animate()

    return () => {
      cancelAnimationFrame(animationId)
      window.removeEventListener('resize', resize)
      window.removeEventListener('mousemove', onMouseMove)
      window.removeEventListener('mouseleave', onMouseLeave)
    }
  }, [])

  return <canvas ref={canvasRef} className="snow-canvas" />
}
